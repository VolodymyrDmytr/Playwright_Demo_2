...
class DynamicTablePage(BasePage):
    ...
    async def check_is_data_as_expected(self) -> bool:
        ...
        # wait till load
        await expect(row_locator.first).to_be_visible()

        # Find CPU line
        logger.debug('Cells count %s', head_cells_locator.count())
        cells_amount = await head_cells_locator.count()

        for i in range(0, cells_amount):
            column_content = await head_cells_locator.nth(i).text_content()
        ...
        rows_amount = await row_locator.count()
        logger.debug('Rows amount: %s', rows_amount)

        for i in range(0, rows_amount):
            row_content = await row_locator.nth(i).text_content()
        ...
        actual_locator = self.locators.table_cell_in_row_locators(row).nth(
            column)
        actual_result = await actual_locator.text_content()

        # Check
        await expect(expected_cpu_locator).to_have_text(
